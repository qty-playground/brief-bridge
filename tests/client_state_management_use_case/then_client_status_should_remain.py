...
def invoke(ctx: ScenarioContext, expected_status: str) -> None:
    """
    Verify client status has not changed from expected value via API
    ...
    assert client_data['status'] == expected_status, f"Client status should remain {expected_status} but was {client_data['status']}"