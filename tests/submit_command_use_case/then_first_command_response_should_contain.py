...
def invoke(ctx: ScenarioContext, expected_response_body: str) -> None:
    """Verify first command response contains expected data"""
    ...
    # Get first command response from results storage
    assert 'first_command_response' in ctx._results, "Should have first_command_response from previous step"
    actual_response = ctx._results['first_command_response']
